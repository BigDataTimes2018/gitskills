...
def createSqlStr_for_word(ent_tax_id, dateTm):
    ...
    sqlStrList.append(sqlStr4)
    #37  上游企业近12个月交易金额合计#37万元，因为是上游的数据所以这里要排除断档的企业的交易金额是不能计算进其中的
    ...
    sqlStrList.append(sqlStr5)
    #42  下游企业 近12个月交易金额合计#42万元，这里问是又是稳定的下游
    ...
    for rowNum in range(df.shape[0]):
        for colNum in range(4):
            if np.isnan(resDataFrame.iloc[rowNum, colNum]) == False:
                ...
                sheet.write(rowNum + 2, colNum + 3, value)
                #        sheet.write(25, 0, 'Dandan Sun')  # 写入excel，第一个值是行，第二个值是列
...
if __name__ == "__main__":
 #   get_table_data(sys.argv[1], sys.argv[2])
    time_start=time.time()
    enterWord(sys.argv[1],sys.argv[2])
#    test_main(sys.argv[1],sys.argv[2])
    time_end=time.time()
    print('----  totally cost '+str(time_end-time_start)+' second  ----\n\n\n\n')
