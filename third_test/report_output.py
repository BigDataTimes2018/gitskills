			...
			for ent in ent_list:
				...
				if(res==1):
					...
					if(res2[0][0]<10): # 断档严重
						...
						print(ent_tax_id+"此家企业数据断档严重，拒绝申报数据导入")
						logger.info(ent_tax_id+"此家企业数据断档严重，拒绝申报数据导入")
						# 如果断档严重，考虑到时间问题，直接进入下一次的循环
						continue
					else:
						# 如果通过这步的数据断档校验，那么就进行申报数据的导入
						ent_ok.append(ent_tax_id)
						report_output_part2(ent_tax_id)
						end_time=datetime.now().timestamp()
						# 	将生成报告的时间插入到数据库中
						total_time = begin_time - end_time
						conn3 = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='yst@2019', db='yst')
						cur3 = conn3.cursor()
						sql3 = "UPDATE yst.loan_report_manage_record SET total_time = " + total_time + \
							   " WHERE ent_name = (SELECT ent_nm FROM rdc.dim_ent_info WHERE ent_tax_id = '" + ent_tax_id + "')"
						res1 = cur3.execute(sql3)
						conn3.commit()
						cur3.close()
						conn3.close()
				else:
...
def report_process(ent_tax_id,begin_date): #返回0，1。0表示拒绝，1表示推送
	...
	s = "replace into loan_report_manage_record values('"+apply_id+"','"+ent_nm+"','YNRRC_GD','"+product_id+"','1','"+\
	file_doc+"','doc','"+str(ori_file_size_doc)+"','"+ori_file_create_time_doc+"','"+"',null,null,null,null,null);"
	executeSQL(s)
	s = "replace into loan_report_manage_record values('"+apply_id+"','"+ent_nm+"','YNRRC_GD','"+product_id+"','0','"+\
	file_xls+"','xls','"+str(ori_file_size_xls)+"','"+ori_file_create_time_xls+"','"+"',null,null,null,null,null);"
	executeSQL(s)
	
	if(score>40):
		# 进行转为pdf之后，用pdf的文章进行加密的
		...
		# executeSQL(updateSQL)
		updateSQL = "update yst.loan_info set apply_status = '6', apply_status_memo = '报告推送成功。'\
		, update_time = current_timestamp() where seria_id ='"+apply_id+"';"
		executeSQL(updateSQL)
		...
		# updateSQL = "update yst.register_auth_info set report_output_flag = -1 where ent_tax_id ='"+ent_tax_id+"';"
		# executeSQL(updateSQL)
		updateSQL = "update yst.loan_info set apply_status = '14' , apply_status_memo = '未通过审核，税控数据评分未达标。'\
		, update_time = current_timestamp() where seria_id ='"+apply_id+"';"
		executeSQL(updateSQL)
...
def report_output_part2(ent_tax_id):
	# 日期参数，用于判断正确的申报日期区间，或计算断档使用日期区间
	year = int(date.today().strftime("%Y"))
	month = int(date.today().strftime("%m"))
	begin1 = (datetime(year=year,month=month,day=1)+timedelta(days=-1)).strftime("%Y-%m")+"-01" # 上月1号
	...
	print(ent_tax_id +" 的申报处理中……")
	logger.info(ent_tax_id+"开始导入申报数据.....")
	...
	if(res==1):
		...
		if(res3[0][0]==end1):
			...
			# 小微企业，要考虑到按季提交申报表的情况。如7月10日时小微企业报表截止到3月31日是正常的。
			# 判断时间差，105表示一个季度加半个月。
			if((datetime.today().date() - datetime.strptime(res3[0][0],"%Y-%m-%d").date()).days<=105):
				...
				tempSQL = "SET SQL_SAFE_UPDATES = 0;"
				executeSQL(tempSQL)
				updateSQL = "update yst.loan_info set apply_status = '4', apply_status_memo = '未通过审核，税控数据评分未达标。'\
				, update_time = current_timestamp() where seria_id ='"+apply_id+"';"
				executeSQL(updateSQL)
